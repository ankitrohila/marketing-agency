    ...
    <>
      ...
      <main style={{ flex: 1, padding: "28px 32px", overflowY: "auto" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
          <p style={{ fontSize: "0.875rem", color: "rgba(255,255,255,0.4)" }}>{categories.length} categories · Categories cannot be deleted to maintain content integrity.</p>
        </div>
        ...
        <div style={{ background: "#141414", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 14, overflow: "hidden" }}>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={5} style={{ padding: "40px", textAlign: "center", color: "rgba(255,255,255,0.3)" }}>Loading...</td></tr>
              ) : categories.map((cat) => (
                  ...
                  <td style={{ padding: "14px 16px" }}>
                    {editing === cat.slug ? (
                      <input
                        ...
                        style={{ background: "#1a1a1a", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 6, color: "#F5F5F5", fontSize: "0.875rem", padding: "6px 10px", outline: "none" }}
                      />
                    ) : (
                      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        <span style={{ width: 10, height: 10, borderRadius: "50%", background: cat.color, flexShrink: 0 }} />
                        <span style={{ fontSize: "0.875rem", fontWeight: 600, color: "#F5F5F5" }}>{cat.name}</span>
                      </div>
                    )}
                  </td>
                  <td style={{ padding: "14px 16px" }}>
                    <span style={{ fontSize: "0.8125rem", color: "rgba(255,255,255,0.4)", fontFamily: "monospace" }}>{cat.slug}</span>
                  </td>
                  <td style={{ padding: "14px 16px", maxWidth: 280 }}>
                    {editing === cat.slug ? (
                      <input
                        ...
                        style={{ width: "100%", background: "#1a1a1a", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 6, color: "#F5F5F5", fontSize: "0.8125rem", padding: "6px 10px", outline: "none" }}
                      />
                    ) : (
                  ...
                  <td style={{ padding: "14px 16px" }}>
                    {editing === cat.slug ? (
                      <div style={{ display: "flex", gap: 6 }}>
                        ...
                        <button onClick={cancelEdit} style={{ padding: "5px 12px", borderRadius: 6, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", color: "rgba(255,255,255,0.5)", fontSize: "0.75rem", fontWeight: 600, cursor: "pointer" }}>Cancel</button>
                      </div>
                    ) : (
                      <button onClick={() => startEdit(cat)} style={{ padding: "5px 12px", borderRadius: 6, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", color: "rgba(255,255,255,0.5)", fontSize: "0.75rem", fontWeight: 600, cursor: "pointer" }}>
                        Edit
                      </button>
                    )}
                  </td>
